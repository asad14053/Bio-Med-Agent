...
class MoELLaVALlamaModel(LlavaMetaModel, LlamaModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        dtype=None,
            ...
            """
            Build a 4D causal attention mask compatible with newer transformers.
            Shape: (batch_size, 1, target_len, source_len)
            """
            import torch

            batch_size, tgt_len = input_shape

            # Figure out device and dtype
            ...
            # Basic lower-triangular causal mask: [tgt_len, tgt_len]
            causal = torch.tril(torch.ones((tgt_len, tgt_len), device=device, dtype=torch.bool))
            # Expand to [1, 1, tgt_len, tgt_len]
            causal = causal.view(1, 1, tgt_len, tgt_len).expand(batch_size, 1, tgt_len, tgt_len)

            if attention_mask is not None and attention_mask.dim() == 2:
                # attention_mask: [batch, src_len] -> [batch, 1, 1, src_len]
                padding = attention_mask[:, None, None, :].to(torch.bool)
                # combine: do not attend to padded positions
                causal = causal & padding

            # Convert bool mask to additive mask (float with -inf where masked)
            if causal.dtype == torch.bool:
                causal = causal.masked_fill(~causal, torch.finfo(dtype).min)
                causal = causal.to(dtype)
